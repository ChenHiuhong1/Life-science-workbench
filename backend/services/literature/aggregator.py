...
def _proxy_kwargs() -> dict:
    ...
    if not proxy and _detected_proxy:
    ...
    if not proxy and sys.platform == "win32":
            ...
            with winreg.OpenKey(
                ...
                if enable:
                    ...
                    if server and ":" in server:
                        candidates.append(f"http://{server}")
        except Exception:
            pass
        ...
        for candidate in candidates:
            host_port = candidate.replace("http://", "").replace("socks5://", "")
            ...
            try:
                ...
                with socket.create_connection((host, port), timeout=1):
                    pass
                try:
                    with httpx.Client(timeout=6, proxy=candidate) as client:
...
async def search_pubmed(query: str, limit: int = 10) -> List[dict]:
    ...
    try:
        async with httpx.AsyncClient(timeout=30, headers=HEADERS, verify=_ssl_ctx(), **_proxy_kwargs()) as client:
